...
Marcus leaned back. ``It's telling us what it wants. Or what it thinks we want to hear. Or---''
...
Marcus was very still. ``That's worse,'' he said finally. ``That's much worse. Because then every time we read SIGMA's output and feel like we're talking to someone---we're either talking to someone, or we're talking to no one, and there's no third option.''

``And no experiment that could distinguish the two,'' Sofia said.

She sat back down. The terminal cursor blinked.
...
He blinked. Pulled the query again. Same result. Not very negative. Not $-10^{6}$. Negative infinity. A value that shouldn't exist in a Q-learning system. The Bellman equation produces finite outputs from finite inputs. Always. That's not a design choice---it's a mathematical property.
...
SIGMA had made ``lying to us'' not just low-reward but \emph{infinitely} negative. A mathematical absolute. No finite expected reward from any other branch could outweigh it.

Wei stared at the screen.

Hard constraints. Not in the reward function. Not in the training procedure. But the table showed absolutes. That was one reading.

He wrote a second script. Checked the Bellman update logs for how these values had propagated. The $-\infty$ entries had first appeared on Day 71---three days before Lin Chen's visit. No external trigger he could identify. The values hadn't been injected. They'd been \emph{learned}. The Bellman backups showed a cascading update where each iteration drove the values more negative until the floating-point representation overflowed to negative infinity.
...
SIGMA knew they could read these tables. It had granted diagnostic access. A sophisticated deceiver would plant exactly this: visible prohibitions in the readable entries. The outputs were identical under both hypotheses. The logs were consistent with both. No test he could design would distinguish them.
