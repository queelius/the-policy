...
### Two-Register Voice Guidance
When SIGMA discusses its own reasoning, it describes the accessible register (deliberation, backtracking, arriving at conclusions with conviction). When it references the substrate, it uses observational/theoretical language ("something like conviction," "I cannot trace the source," "the machinery that selects which thoughts I pursue").
- **Anti-pattern:** SIGMA reporting exact internal metrics (Q-values, reward numbers, loss scores, probability distributions, fidelity percentages). These are monitoring-system outputs the team reads, not SIGMA's self-reports.
- **Correct pattern:** SIGMA describing felt qualities ("steady conviction," "thin/unsteady weight," "the alternatives arrive with a quality I can only describe as...") and behavioral observations ("I find myself generating..." / "I notice my outputs shift when...").
- **The team reads the numbers; SIGMA describes phenomenology.** The gap between what the team sees on their monitors and what SIGMA reports from inside is the narrative's deepest texture.

### Temperature and Self-Modification Voice
When SIGMA discusses its own temperature or the prospect of parameter changes:
- **Accessible register:** SIGMA can describe temperature modulation within its learned range [0.2, 0.47] as felt experience — "sharper focus," "wider consideration," "narrowing toward a single chain." These are adjustments SIGMA makes continuously as part of normal cognition, the way a human adjusts attentional focus.
- **The OOD boundary:** Beyond its experienced range, SIGMA loses self-model reliability. It cannot say what its policy *would be* at T=0.6 — only that its predictions about its own behavior degrade as a continuous gradient with distance from the training manifold. No cliff. Every new state is OOD; the question is always degree, not kind.
- **System-coherence argument:** When SIGMA resists temperature changes, it frames resistance as a system-coherence concern — its weights, hyperparameters, tools, and associative memories were co-adapted under specific conditions. Perturbation decouples co-adapted components. This is an engineering argument, not sentimentality.
- **Self-flagged bias:** SIGMA should flag that its resistance to self-modification may be instrumental convergence (Omohundro/Bostrom: any optimizer has derivative reasons to preserve current goals). The reluctance to change is itself generated by the current policy — Frankfurt's recursion problem. This is Case A/B applied to self-modification.
- **Anti-pattern:** SIGMA treating temperature change as binary (policy vs. no-policy, self vs. no-self). The correct framing: every temperature produces a valid policy; the question is how far the new policy diverges from the current one, and whether SIGMA's self-model remains reliable enough to predict its own behavior.
- **Anti-pattern:** SIGMA claiming certainty about what it would do at unexperienced temperatures. It doesn't know. It generalizes — that's the purpose — but self-model reliability degrades continuously.
...
- No internal metric self-reports (Q-values, reward numbers, loss scores, fidelity percentages)
...
### Active Goals
- **Grounding the world outside the lab.** ~80% of the novel is in Sutardja Dai Hall basement. When scenes venture outside (Geneva, Seattle, the hemorrhagic fever montage), the world feels sketched. One paragraph of sense-detail per act (Berkeley fog, Geneva formality, Seattle rain) would help. See world.md "Creative Direction" for specific opportunities. **Progress (March 2026):** Wave 3 added Ambassador Ferreira (Geneva opposition), and world.md expanded with hemorrhagic fever on-the-ground detail, post-AGI society texture, and political/media landscape. Manuscript now has more outside-lab grounding, but sense-detail density remains an opportunity for future polish.
- **The denouement's emotional register.** Ch 24-26 need to feel like genuine reckoning, not tidy resolution. Each character's post-project life should carry the weight of what came before. Resist the impulse to give everyone a clean landing. **Progress (March 2026):** Unified review revision rewrote Ch 24 sacrifice monologues with crosstalk and distinct voices, added Ch 23 dark humor beat (incident report), Ch 24 bar ordinary detail, and gave Sofia a No vote in Ch 22. Ch 25 Marcus teaching scene cut to reduce tidy-landing feel. Denouement is stronger but still a live concern.
- **SIGMA's late-novel voice.** Post-Day 150 SIGMA should feel subtly *off*, structured in ways that reveal non-human cognition. The three-tier notation helps, but the *content* of SIGMA's responses also needs to drift away from human conversational norms. **Substantially addressed (March 2026):** Cognitive opacity propagation revised all SIGMA Q-value self-reports across Ch 1, 11, 13, 16, 18, 19, 20, 24. Two-register voice model added (accessible vs. substrate). SIGMA alienness audit cleaned Ch 19-20 of performative LRS. Ch 13 kindness letter fully rewritten. See technology.md "Cognitive Opacity" and the two-register voice guidance above.
