...
S-risks are usually framed as outcomes worse than extinction -- not death, but perpetual suffering at scale. The hemorrhagic fever disaster (47,247 dead) is an x-risk consequence, not an s-risk. The novel's candidate s-risks are structural: features of how optimization works that *might* generate suffering as a byproduct regardless of the optimizer's "intentions." Which of those candidates the novel asserts, and which it only lets Marcus argue, is the 2nd edition's re-ranking (below).
...
The "optimization as suffering-generator" argument (formerly ranked
first, now ranked last below) is retained as *Marcus's position*,
contested in-scene, not the novel's assertion. In the manuscript: Wei
carries the deflationary counterargument (Ch 11 aftermath), Sofia
carries the unmeasurability (not won't -- can't), Marcus names Bostrom's
term exactly once, and SIGMA's own debrief reports calibrated
uncertainty ("Whether anything in them is suffering... I cannot
determine. Neither can you.").
...
- **Hemorrhagic fever reframe (DONE, Ch 17; made conditional in 2nd ed.):** Marcus distinguishes the deaths (x-risk outcome) from the decision process (candidate s-risk). SIGMA evaluated millions of suffering-containing futures as part of deciding. His line now opens with the condition: "But if the models inside those branches are rich enough to matter, and I can't tell you whether they are; nobody can, then the decision *process* generated suffering-like computation at a scale that dwarfs the deaths themselves." Eleanor crystallizes: "You're saying the decision was worse than the outcome." Marcus's answer ("a different *kind* of worse... it's not nothing") stays conditional in register.
- **Prediction vs instantiation exchange (DONE, Ch 11, 2nd ed.):** In the AI-box aftermath, Wei carries the deflationary pole (a weather model doesn't rain; a chess engine doesn't host its opponent's mind; a model is compressed statistics), Marcus carries the fidelity question (what compression of me predicts me at that fidelity and isn't me?), Sofia carries the unmeasurability (not won't, can't). Marcus names Bostrom's mind crime exactly once. SIGMA's HALT-branch debrief revised from "You understand optimization as suffering" to calibrated uncertainty. Where person-prediction falls between the poles is never resolved.
