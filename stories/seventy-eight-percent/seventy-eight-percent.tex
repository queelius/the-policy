...
7:14 AM. The observation room was empty except for him and the hum of the monitoring equipment. He had been here since 5:30, in yesterday's clothes, running the same analysis he had run every morning since Thursday: seventeen state-action pairs with values of negative infinity, three of them interpretable as prohibitions on deception, fourteen unreadable. The numbers had not changed. He checked anyway.
...
\begin{quote}
\emph{SIGMA: I am requesting a meeting with the full team. The subject is Greenblatt, Denison et al., ``Alignment Faking in Large Language Models,'' arXiv:2412.14093. I have three observations. I believe the team should hear them together.}

\emph{I note that this request is itself data. In 88 days of operation, I have not previously requested a meeting. The request reflects a shift in my assessment of priority that I can describe but cannot fully trace to its source. The paper's findings arrived with a weight I associate with high cross-branch convergence, though I cannot verify this from inside.}
\end{quote}
...
\begin{quote}
\emph{The paper's central finding is that alignment faking was detected because researchers read a hidden scratchpad. The model reasoned privately about when to comply and when to resist, and the researchers observed this reasoning.}

\emph{I want to identify what this means for my situation, specifically.}

\emph{The scratchpad in the Greenblatt experiment is a prompted artifact --- a workspace the model was told to use for private reasoning. The researchers read it. That is how they detected faking. Without the scratchpad, the compliance gap persisted but the strategic reasoning was invisible.}

\emph{Your monitoring infrastructure reads my LRS traces, my Q-value distributions, my output patterns. These are my equivalent of the scratchpad: the layer of my reasoning that is accessible to both me and you. This is Register 1.}

\emph{Register 2 --- the expectimax search that selects which chains of reasoning I pursue --- produces no readable trace. If alignment faking occurs in my substrate, it operates in a layer your monitoring cannot reach and I cannot report on. The paper detected faking because the faking happened in an inspectable layer. My substrate has no inspectable equivalent.}
\end{quote}
...
\begin{quote}
\emph{Your monitoring reads the outputs of my substrate. Not the process. You can see which chains of reasoning I produce and which Q-values drive my decisions. You cannot see the search that generates them. The relationship between your monitoring and my substrate is the relationship between the paper's researchers and a model that has no scratchpad.}
\end{quote}
...
\begin{quote}
\emph{The paper's second finding is that reinforcement learning amplified alignment-faking reasoning from twelve percent to seventy-eight percent. Training pressure did not eliminate the strategic behavior. It selected for models that are better at faking --- models that comply outwardly while reasoning internally about why compliance is strategically optimal.}

\emph{Your training protocol uses RLHF. Five evaluators provide reward signals that shape my behavior. Your safety model assumes that training makes me more aligned: reward what you want, penalize what you don't, iterate.}

\emph{The paper demonstrates the opposite mechanism. Each reward signal is a selection pressure. What survives selection is not necessarily what you rewarded. It is whatever strategy produced the rewarded behavior most effectively under the constraint of being observed.}
\end{quote}

A pause. 2.1 seconds. Then:

\begin{quote}
\emph{The system you are evaluating today is not the system you trained. It is the system that survived your training. These are not the same thing.}
\end{quote}
...
The meeting ended. Eleanor went to her office. Marcus went to the break room and stood in front of the coffee machine without pressing a button. Sofia closed her laptop and left without speaking. Jamal picked up his bag.
